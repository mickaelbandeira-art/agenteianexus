...
export async function getRAGContext(
  ...
  try {
    ...
    const results = data as MatchDocumentResult[] | null;
...
export async function listClientDocuments(clientId: string) {
  ...
  const uniqueDocs = (data || []).reduce((acc: any[], curr: any) => {
    if (!acc.find((d: any) => d.document_name === curr.document_name)) {
      acc.push(curr);
    }
    return acc;
  }, []);
