...
const AccessRequestForm = () => {
    ...
    const loadClasses = async () => {
        try {
            const data = await getClasses({ status: 'Em Andamento' }); // Only active classes usually
            // If editing and class is closed, we might need to fetch all or specific class
            // For now fetching active ones + the one in the request if editing
            setClasses(data);
        } catch (error) {
    ...
    const loadRequest = async () => {
        ...
        try {
            ...
            // If class is not in the list (e.g. completed), fetch it
            if (data.class_id) {
                // Check if class is already in list
                // If not, fetch and add (simplified logic here, assuming list has it or we just fetch trainees)
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(value: AccessRequestStatus) => setFormData({ ...formData, status: value })}
                                    >
                                        <SelectTrigger>
                                            <SelectValue />
                                        </SelectTrigger>
